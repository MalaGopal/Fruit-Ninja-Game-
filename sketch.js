...
function draw() {
  ...
  sword.x=World.mouseX;   
   ...
   if(fruitGroup.isTouching(sword)) {
    ...
    textSize(30);
    fill("Red")
    text("Press Y if you want to restart the game.", width/3,50)

    if(keyDown("Y") && gameState === END){
      gameState = PLAY;
      score = 0
      sword.addImage(swordImageyy);
    }
  }
...
function fruits() {
  ...
  if(World.frameCount%80 === 0) { 
     fruit=createSprite(width,20,20,20);
     fruit.scale=1;
     ...
     fruit.y=Math.round(random(50,height-100));
     fruit.lifetime=200;
...
function Enemys() {
   ...
   if(World.frameCount%200 === 0) { 
     ...
     monster1 = createSprite(0,Math.round(random(100,height-100)),20,20);
   ...
   if(World.frameCount%100 === 0) {
     ...
     monster2 = createSprite(width,Math.round(random(100,height-100)),20,20);
